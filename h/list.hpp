...
public:
    void *operator new(size_t size) { return MemoryAllocator::_mem_alloc(size/MEM_BLOCK_SIZE+(size%MEM_BLOCK_SIZE ? 1:0)); }
    void operator delete(void *ptr) { MemoryAllocator::_mem_free(ptr); }
    List() : head(0), tail(0) {}
